...
se pueda ejecutar con facilidad el código
"""

catName='LEULTR'

params=OrderedDict(
        model=['Resnext50', 'Resnet18','Coatnet'], #modelos a probar, integrados en el script Modelos.py
        optim=['SGD', 'Adam'], #En la clase AgoModel se configuraron los optimizadores, de tal forma
                       #que el usuario solo ponga una cadena como entrada, y el script
                       #identifica en PyTorch nativo el optimizador a trabajar
        ridge=[0.01], #Es el término denominado "alfa" en la regularización Rdige
                                   #que escala la penalización
        lr=[0.0001, 0.00001], #es el learning rate con el que se actualizarán los parámetros del modelo
        ...
        batch=[64], #El tamaño del lote, que se especifica en el DataLoader
...
if __name__=='__main__':
    for run in RunBuilder().get_runs(params):
        ...
        if train_dataset.class_to_idx==val_dataset.class_to_idx:
            ...
            train_dataset = FilteredImageFolder(train_dataset.root,transform=transformations)
            val_dataset = FilteredImageFolder(val_dataset.root,transform=transforms_val)
            assert train_dataset.class_to_idx==val_dataset.class_to_idx

        

        train_loader = DataLoader(train_dataset, batch_size=run.batch,num_workers=8,shuffle=True)
        val_loader = DataLoader(val_dataset, batch_size=run.batch,num_workers=8,shuffle=False)
        # model
